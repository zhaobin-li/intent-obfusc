...
g + labs(x = dist_lab, y = glue("p(Success) {norm_axy(params$norm)}")) +
  scale_x_continuous(breaks = unique(success_expanded_data$bbox_dist), labels = scales::label_percent()) +
  scale_color_viridis_c(name = len_lab, breaks = unique(success_expanded_data$bbox_len), labels = scales::label_percent())
```
