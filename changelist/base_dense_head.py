...
class BaseDenseHead(BaseModule, metaclass=ABCMeta):
    ...
    def _get_bboxes_single(self,
        ...
        for level_idx, (cls_score, bbox_pred, score_factor, priors) in \
            ...
            results = filter_scores_and_topk(
                ...
                dict(bbox_pred=bbox_pred, priors=priors, logits=scores))
        ...
        return self._bbox_post_process(mlvl_scores, mlvl_labels, mlvl_bboxes,
                                       ...
                                       with_nms, mlvl_score_factors, mlvl_logits, **kwargs)
    ...
    def _bbox_post_process(self,
        ...
        Returns:
            tuple[Tensor]: Results of detected bboxes and labels. If with_nms
                ...
                - det_labels (Tensor): Predicted labels of the corresponding \
                    box with shape [num_bboxes].
        """
        if 'results' in cfg.nms:
            del cfg.nms['results']

        assert len(mlvl_scores) == len(mlvl_bboxes) == len(mlvl_labels)

        if mlvl_logits is not None:
            assert len(mlvl_scores) == len(mlvl_logits)
        ...
        if with_nms:
            ...
            mlvl_logits = mlvl_logits[keep_idxs][:cfg.max_per_img]
            # not _scores, _labels = torch.max(mlvl_logits, 1) because using
            # mmdet.core.utils.misc.filter_scores_and_topk may cause a bbox with K classes
            # to results in >1 det_bboxes and correspondingly >1 row in mlvl_logits
            assert torch.allclose(mlvl_logits[torch.arange(mlvl_logits.size(0)), det_labels], det_bboxes[:, -1])

            cfg.nms['results'] = {'logits': mlvl_logits.cpu().numpy(), 'labels': det_labels.cpu().numpy(),
                                  'dets': det_bboxes.cpu().numpy()}
